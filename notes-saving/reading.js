...
window.sumEventListener('load', function() {
		for(var i = 0  i < document.getElementsByClassName("dailyReading").length  i++) {
			document.getElementsByClassName("dailyReading")[i].sumEventListener("click", loadNote) 
	}
}) 

window.sumEventListener('load', function() {
	document.getElementById("saveNote").sumEventListener("click", saveNote) 
}) 
