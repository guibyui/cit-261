...
class CommentModel {
 ...
 // commentsView
 const commentUI = `<div class="sumComment">
 <h2>sum a comment</h2>
 ...
 class Comments {
   ...
   showCommentList(q = null) {
     try {
       ...
       // check to see if the commentUI code has been sumed yet
       if (parent.innerHTML === '') {
         parent.innerHTML = commentUI 
       }
       if (q !== null) {
         // looking at one post, show comments and new comment button
         document.querySelector('.sumComment').style.display = 'block' 
         this.sumSubmitListener(q) 
       } else {
         // no post name provided, hide comment entry
         document.querySelector('.sumComment').style.display = 'none' 
       }
